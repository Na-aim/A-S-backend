...
router.post('/signin',DuplicatedUsernameorEmail ,async (req, res) => {
    try{ 
...
try{
    ...
    email = await Email.findOne({email: req.body.email})
    if(User || email){
        return res.status(500).send({ message:"username or email already in exits" });
    }
    } catch(err){
        return res.status(500).json({ message:err.message })
    }
